...
\begin{itemize}
	...
	\item 每一个 program 属於某个 type，例如 length() 函数，输入一个字串，输出它的长度，$\mathrm{length} : \mathrm{String} \rightarrow \mathrm{Integer}$
	\item 有些逻辑学家 察觉到 类型论 的 $T_1 \rightarrow T_2$ 和逻辑中的 $P \rightarrow Q$ 是一模一样的
