...
首先考察一下 fuzzy truth value 在各种情况下的正确性：
\begin{itemize}
\setlength{\itemsep}{0.2ex}  % this works locally
	\item 每个 fuzzy proposition 是一个 set, 里面有 该命题的「证明」
	\item 例如「人类」的集合是「有人类」这个命题的证明
	\item 而，「数学家」作为「人类」的子集，等於命题「所有人都是数学家」的证明
\end{itemize}
而这和 fuzzy value 的诠释是一致的。

「人」的概念由若干个 \emp{属性} (properties) 表示：
\begin{equation}
...
\begin{itemize}
	\item 还有这跟 ``Marilyn Monroe is sexy'' 是不是一致？
	\item Marilyn 的所有属性集合，其中 imply sexy 的 subset 
	\item 还是 sexy 的所有属性集合，其中 Marilyn 也有的？
\end{itemize}
