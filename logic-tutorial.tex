...
Architecturally, the AI is a \emp{dynamical system} that constantly updates its ``state'' $\vect{x}$ via: \footnote{Part of the state $\vect{x}$ contains \emp{sensory input} and \emp{action output} that allow the AI to interact with the external environment.}
\begin{equation}
...
深度学习是现时最强的学习算法，它可以学习非常复杂的\textbf{非线性函数}。 问题是怎样使用这件「武器」。 我提出的 architecture 就是 (\ref{eqn:AGI-architecture})，这也是 Richard Sutton 提出的基於 \emp{强化学习} 的模型。 \footnote{AGI 的架构还可以包括 episodic memory 等部分，现在我们考虑的是 minimalist architecture.  如果不用高度抽象的理论，很多时会迷失在支节里。 }

同时，我提议 利用 \textbf{逻辑结构}，\textbf{约束 $\vect{F}$ 的搜寻空间}（这叫 inductive bias），令它可以更快地学习到人类水平的智能。 这是不是达到 AGI 的最好的方法呢？ 不实践是无法知晓的。 Sutton 甚至认为，不需要人为的 bias，纯粹增加\textbf{计算力}就可以了。 
\begin{equation}
\nonumber
...
它已经被发现了很多次，实际上它的发现者包括： Brouwer-Heyting-Kolmogorov-Sch\"{o}nfinkel-Curry-Meredith-Kleene-Feys-G\"{o}del-L\"{a}uchli-Kreisel-Tait-Lawvere-\\
Howard-\mbox{de Bruijn}-Scott-Martin-L\"{o}f-Girard-Reynolds-Stenlund-Constable-\\
Coquand-Huet-Lambek ....
...
更详细观察，table (\ref{truth-table:material-implication}) 里面 $A$ 和 $B$ 的 truth values 可以看成是它们的 types 有没有 \textbf{inhabitants}. Type $A$ 的 inhabitant 就是它的证明 $\square$，没有证明就是 $\emptyset$.  或者推广到：命题 $A$ 的真值 = $A$ 的 type 作为 集合 的 \textbf{cardinality}; The truth valuation of $A = |A|$.  这样看，$A \Rightarrow B$ 的真值 就是 $|B^A|$, 亦即是从 $\{\square\}$ 或 $\emptyset$ 到 $\{\square\}$ 或 $\emptyset$ 的 map, 而这个 map 只有在 $\emptyset \mapsto \{\square\}$ 的时候是空集（不可能）。
...
\begin{equation}
\nonumber
\mbox{Joachim Lambek (1922-2014)} \quad \vcenter{\hbox{\includegraphics[scale=0.45]{Lambek.jpg}}}
\end{equation}
将 type theory 对应到 category theory，这工作是 Lambek 做的，於是完成了 Curry-Howard-Lambek 的「三位一体」：
\begin{equation}
\begin{tikzcd}[column sep = small]
\boxed{\mbox{logic}} \arrow[rr, dash] \arrow[dr, dash]
&& \boxed{\mbox{programming}} \arrow[dl, dash] \\
& \boxed{\mbox{category theory}}
\end{tikzcd}
\end{equation}
...
X \arrow[r, "!"] \arrow[d, tail, "m"] & 1 \arrow[d, "\mathrm{true}"] \\
Y \arrow[r, "\Chi_m"] & \Omega
\end{tikzcd}
\end{equation}
其中：
\begin{itemize}
...
\vcenter{\hbox{\includegraphics[scale=0.6]{Kripke.jpg}}}
\end{equation}
...
\vcenter{\hbox{\includegraphics[scale=0.35]{Voevodsky.jpg}}}
\end{equation}
