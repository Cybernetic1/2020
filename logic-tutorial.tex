...
或者用一个男人更容易想像的例子： 当我们说「玛丽莲梦露 很性感」，这个命题的 fuzzy truth value 就是「Marilyn 的属性」和「性感的属性」之间的某种 \textbf{共同性}的量度。  这种 truth value 的定义 是基於 \textbf{intension} 而不是 \textbf{extension}.
...
\end{itemize}
但在某些情况下，$A \mbox{ is } B$ 但 $A$ 不是\textbf{集合}（例如 John 或 Marilyn 不是集合），则 extensional 的诠释 不适用。 

关於 fuzzy truth value 还有个问题： 一个对象的 \textbf{属性} 在某程度上可以随意增加或减少，因此纯粹 计算 属性的\textbf{数量}，并不能客观地决定 truth value.  似乎每个 属性 应该有某种 \emp{测度} (measure) 或 \emp{权重} (weight), 这样在改变 属性 的描述时，fuzzy truth value 保持\textbf{不变}。

\subsection{Fuzzy implication}
\label{sec:fuzzy-implication}

Implication 问题: fuzzy implication 并不对应於 material implication in Boolean algebra.
