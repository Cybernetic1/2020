...
Topos 中 $\rightarrow \Omega$ 这个箭头，是 \emp{classifying space} 或 \emp{moduli space} 的一个特例。  这方面我不太熟悉。
...
在 数理逻辑／集合论 里，forcing 指的是一些 加在某 集合 $G$ 上的 \textbf{条件}，说某些 元素 属於 或不属於 $G$.  这些 条件 用逻辑表达，例如：
\begin{equation}
\{ 3 \in G, 57 \notin G, 873 \notin G \}
\end{equation}
is a condition.

这样的一个条件 $c$ 可以「强迫」集合 $G$ 里面至少有 100 个质数，或至少有 1万个质数。 
