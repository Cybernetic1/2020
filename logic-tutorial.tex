...
It turns out that $\exists$ and $\forall$ are \textbf{adjoints} to the map $*$:
\begin{equation}
\begin{tikzcd}[column sep = large]
\mbox{Forms}(\vec{x}) 
\arrow[r, "*"]
& \mbox{Forms}(\vec{x},y) 
\arrow[l, shift right=1.2em, "\exists y"'] % "\bot"
\arrow[l, shift left=1em, "\forall y"] % "\bot"'
\end{tikzcd}
\end{equation}
or simply, $\exists \dashv * \dashv \forall$.  而这是十分 makes sense 的，因为 一个 $\phi(\vec{x},y)$ 的式子，经过 $\forall y. \; \phi(\vec{x},y)$ 之后，就变成一个和 $y$ \textbf{无关}的式子。
...
For each $A \in \mathcal{A}$ , we have a functor $\mathcal{A} \rightarrow \mathbf{Set}$. \\
Putting them all together gives a functor $\mathcal{A}^{\mathrm{op}} \rightarrow [\mathcal{A} , \mathbf{Set}]$. \\
For each $A \in \mathcal{A}$ , we have a functor $\mathcal{A}^{\mathrm{op}} \rightarrow \mathbf{Set}$. \\
Putting them all together gives a functor $\mathcal{A} \rightarrow [\mathcal{A}^{\mathrm{op}} , \mathbf{Set}]$. 

\begin{equation}
\begin{tikzcd}[column sep = 20ex]
\mathcal{A}^{\mathrm{op}} 
\arrow[r, shift left, "H_A"]
\arrow[r, shift right, swap, "X"]
& \mathbf{Set}
\end{tikzcd}
\end{equation}
...
在 命题层面 还有一个很重要的 \emp{product-hom adjunction},  它说的是 $A \wedge B$ 和 $A \Rightarrow B$ 之间的邻接：
\begin{equation}
