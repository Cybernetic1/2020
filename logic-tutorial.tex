...
另一个方法是 借助 $T$ 定义\footnote{This formula is taken from \parencite{Abramsky2011} }： （注意 在下图中 $\bar{X}$ 和 $Y$ 变成平面）
...
\end{eqnarray}
上式可以这样理解： （下图中 $\delta$ 是「杀掉」$Y$ 的 projection）
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=0.7]{Lawvere-cylindrification-exists.png}}}
\end{equation}
但 (\ref{eqn:Abramsky-1}) 似乎比较 general，因为 $\delta$ 可以是任何 $Y \rightarrow X$ 的映射。 这或许可以帮助我们 推广到更为一般的 quantifiers，但我暂时未有考虑。 \\
类似地：
\begin{eqnarray}
\forall T \subseteq \bar{X}: \qquad 
