...
\item \cc{我的想法是重复这个思路，但引入 symmetric NN 的结构}
...
\begin{itemize}
	...
	\item \cc{下面会看到，其实这个方法也未必需要，因为 BERT 的「注意力机制 」也可以用来做 逻辑推导}
	{Below we'll see that this may not be necessary, as BERT's \emp{attention mechanism} can also be used to perform logic inference}
...
\item \cc{根据 Curry-Howard isomorphism，BERT 的 ``transform'' 其实对应於某种 \emp{另类的逻辑}，这种逻辑的好处是运行\emp{非常快}}
{By the Curry-Howard isomorphism, BERT's ``transform'' corresponds to some kind of \emp{alternative logic}, which has the advantage of \emp{very fast} execution}
