...
\begin{itemize}
	...
	\item 下面会看到，其实这个方法也未必需要，因为 BERT 的「注意力机制 」也可以用来做 逻辑推导
...
{Analogously, attention on higher levels may process relations among propositions}
...
\item BERT 的做法是： 每次只输出 $N$ 个命题，而前提的 ``support'' 也是上一层的 全部 $N$ 个命题，每个前提的「影响力」由 matrix 权重决定
...
\begin{itemize}
	...
	\item 逻辑化 建立了 知识图谱 和 BERT 之间的一道桥梁
\end{itemize}
\end{frame}
